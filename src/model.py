...
State = int  # 0 or 1
PhiFn = Callable[[int, State], float]                 # phi(i, xi)
PsiFn = Callable[[int, int, State, State], float]     # psi(i, j, xi, xj)
...
class GraphModel:
    ...
    """
    n: int
    edges: List[Tuple[int, int]]  # undirected edges (i,j) with i<j recommended
    phi: PhiFn
    psi: PsiFn
...
def build_neighbors(n: int, edges: List[Tuple[int, int]]) -> Dict[int, List[int]]:
    ...
    for a, b in edges:
        if a == b:
            raise ValueError("Self-loop not supported in this project.")
